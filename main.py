...
def save_user_data(data):  # сохраняет словарь с классом в файл
...
def send_welcome(message):  # первое сообщение бота при новом чате
    ...
    if user_id in user_class:  # у пользователя уже выбран класс
...
def get_today_name():  # получение сегодняшнего дня
    days_ru = ["Понедельник", "Вторник", "Среда", "Четверг", "Пятница", "Суббота", "Воскресенье"]
    return days_ru[datetime.now().weekday()]


@bot.callback_query_handler(func=lambda call: True)
def callback_query(call):  # обработчик все нажатия на кнопки, обработка выбора дня и возврата в главное меню
    user_id = str(call.from_user.id)
    chat_id = call.message.chat.id
    message_id = call.message.message_id
    if call.data.startswith("day_"):
        day = call.data[4:]  # получаем название дня
        if user_id not in user_class:
            bot.send_message(chat_id, 'Сначала выберите класс через кнопку "Сменить класс".')
...
def schedule(message):  # отправляет расписание на текущий день
    user_id = str(message.from_user.id)
    if user_id not in user_class:
        bot.send_message(message.chat.id,
                         "Сначала выберите ваш класс с помощью /setclass", reply_markup=get_class_keyboard())
        return
    class_name = user_class[user_id]
...
def handle_schedule_button(message):  # текст сообщения с расписанием
    user_id = str(message.from_user.id)
    if user_id not in user_class:
...
def handle_change_class_button(message):  # текст сообщения со сменой класса
    set_class_command(message)


@bot.message_handler(func=lambda message: message.text == "🌤 Погода")
def handle_weather_button(message):  # текст сообщения с запросом погоды
        try:
...
def set_class(message):  # сохраняет выбранный класс для пользователя
    user_id = str(message.from_user.id)
    selected_class = message.text
    user_class[user_id] = selected_class
    save_user_data(user_class)
    bot.send_message(message.chat.id, f"Вы выбрали класс {selected_class}. Теперь вы можете использовать /schedule для получения расписания.",
                     reply_markup=main_keyboard())  # убрать клавиатуру


@bot.message_handler(commands=['setclass'])
def set_class_command(message):  # показывает клавиатуру выбора класса
    bot.send_message(message.chat.id, "Выберите ваш класс:", reply_markup=get_class_keyboard())

def run_bot(): # запускает бота в режиме long polling
...
if __name__ == '__main__':
    ...
    # запуск Flask-сервера на порту, который даёт Render
